/**
 * Formatea el nombre del módulo para la UI: quita el sufijo " (SP)".
 * @param {string} nombre - Nombre del módulo (backend)
 * @returns {string} Nombre para mostrar (ej. "Tema 1 (SP)" → "Tema 1")
 */
export const formatModuloNombreForDisplay = (nombre) => {
  if (!nombre || typeof nombre !== 'string') return nombre || '';
  return nombre.replace(/\s*\(SP\)\s*$/, '').trim();
};

/**
 * Formatea el nombre de la asignatura para la UI (oculta " | Segunda Promoción" / " | Primera Promoción").
 * @param {string} nombre - Nombre de la asignatura (backend)
 * @returns {string} Nombre formateado para la UI
 */
export const formatAsignaturaNombreForDisplay = (nombre) => {
  if (!nombre || typeof nombre !== 'string') return nombre || '';
  return nombre.replace(/\s*\|\s*Segunda Promoción\s*$/i, '').replace(/\s*\|\s*Primera Promoción\s*$/i, '').trim();
};
