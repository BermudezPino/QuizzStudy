...
function QuizPageContent({ tipo }) {
  ...
  // Convertir a número los IDs de la URL (fallidas mantiene string)
  const asigId = asignaturaId === 'fallidas' ? 'fallidas' : parseInt(asignaturaId, 10);
  const modId = moduloId === 'todos' || moduloId === 'examen' || moduloId === 'favoritos' || moduloId === 'fallidas'
    ? moduloId
  ...
  useQuizLoader({
    ...
    continueFromPending,
    asignaturaId,
    enabled: !isFavoritesQuiz && !isFallidasQuiz
  });
  ...
  // Renderizar loader de preguntas fallidas si es necesario
  if (isFallidasQuiz && (!contextTipoQuiz || contextTipoQuiz !== 'fallidas' || !tipoQuiz)) {
    return (
      <Layout>
        <QuizHeader
          getNombreModulo={() => 'Preguntas fallidas'}
          handleExit={handleExit}
          asignaturaId={asignaturaId}
        />
        <div className="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
