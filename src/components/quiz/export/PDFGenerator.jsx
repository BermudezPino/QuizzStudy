...
import { formatTimestamp, formatAsignaturaNombreForDisplay } from '@utils/quizUtils';
...
const PdfDocument = ({ preguntas, respuestas = {}, asignatura, titulo, subtitulo, mostrarRespuestasUsuario }) => (
  <Document>
    <Page size="A4" style={styles.page}>
      ...
      <Text style={styles.subtitle}>Asignatura: {formatAsignaturaNombreForDisplay(asignatura.nombre)}</Text>
      ...
      <Text style={styles.footer}>
        Documento generado desde QuizzStudy - {new Date().toLocaleDateString()}
      </Text>
      <Text
...
export default function PdfGenerator({
  ...
  const handleGeneratePDF = async () => {
    try {
      ...
      const filename = `${formatAsignaturaNombreForDisplay(asignatura.nombre).replace(/\s+/g, '-').toLowerCase()}-preguntas-${Date.now()}.pdf`;
