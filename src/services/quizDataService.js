 ...
 * Fuente única de verdad: asignaturasPorAno y búsqueda por ID desde @/data.
...
import {
  asignaturas,
  asignaturasPorAno,
  obtenerAsignaturaPorId as obtenerAsignaturaPorIdDesdeData,
} from "@/data";

// Importaciones estáticas de todos los módulos de asignaturas (para fallback y compatibilidad)
...
// Mapa de módulos para acceso rápido por ID (fallback y compatibilidad con código que depende de IDs 1-7)
const modulosMap = {
 ...
 * Usa la fuente única de datos (O(n) sobre las claves de asignaturasPorAno).
...
const buscarAsignaturaPorId = (asignaturaId) => {
  const asignatura = obtenerAsignaturaPorIdDesdeData(asignaturaId);
